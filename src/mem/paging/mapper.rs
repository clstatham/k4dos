...
use crate::{mem::{addr::{VirtAddr, PhysAddr}, allocator::{alloc_kernel_frames, AllocationError}}, util::{KResult, KError}, kerr};

use super::{units::{Page, Frame, AllocatedPages, AllocatedFrames, MappedPages}, table::{PageTable, PageTableEntry, PagingError}};
...
impl<'a> Mapper<'a> {
    ...
    pub fn map_to_single(&mut self, page: Page, frame: Frame, flags: PageTableFlags) -> KResult<(), PagingError> {
        ...
        if !entry.is_unused() {
            return Err(kerr!(PagingError::PageAlreadyMapped(entry.frame().unwrap())));
        }
    ...
    pub fn map_to(&mut self, pages: AllocatedPages, frames: AllocatedFrames, flags: PageTableFlags) -> KResult<MappedPages, PagingError> {
        assert_eq!(pages.size_in_pages(), frames.size_in_pages(), "Number of pages must equal number of frames");
    ...
    pub fn map(&mut self, pages: AllocatedPages, flags: PageTableFlags) -> KResult<MappedPages, PagingError> {
        let frames = alloc_kernel_frames(pages.size_in_pages()).map_err(|e| kerr!(PagingError::FrameAllocationFailed(e)))?;
