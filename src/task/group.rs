...
impl TaskGroup {
    pub(super) fn new(sched: Arc<Scheduler>, pgid: PgId) -> Arc<SpinLock<TaskGroup>> {
        let pg = Arc::new(SpinLock::new(TaskGroup { sched, pgid, tasks: Vec::new() }));
    ...
    pub fn remove(&mut self, task: &Weak<Task>) {
        ...
        if self.tasks.is_empty() {
            self.sched.task_groups.lock().remove(&self.pgid);
        }
    }
    ...
    pub fn signal(&mut self, signal: Signal) {
        for task in self.tasks.iter() {
            ...
            self.sched.send_signal_to(task.upgrade().unwrap(), signal);
        }
    }
}
